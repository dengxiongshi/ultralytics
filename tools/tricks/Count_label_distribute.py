...
image_postfix = ["jpg", "jpeg", "png", "bmp", "tif", "tiff"]

root = Path(r"H:\datasets\train_data_20260826")

split_names = ["train", "val", "test"]

images_folder_path = [str(root / "images" / s) for s in split_names]
labels_folder_path = [str(root / "labels" / s) for s in split_names]

classes = ["斑马", "天使"]
# classes = ['people', 'bicycle', 'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor']
object_info = [32 * 32, 96 * 96]
# object_info = [0.03, 0.1, 0.25]
# object_info = [0.01, 0.04, 0.16]
...
def show_dataset_info_ratio(image_label_dict, visual_box=False, save_path='visual_box'):
    if visual_box and not os.path.exists(save_path):
        os.makedirs(save_path)

    classes_dict = {cls: {'tiny':0, 's': 0, 'm': 0, 'l': 0, 'num': 0} for cls in classes}
    for label_path in tqdm.tqdm(image_label_dict):
        ...
        for cls_id, x_c, y_c, width, height in label:
            ...
            if ratio <= object_info[0]:
                classes_dict[classes[int(float(cls_id))]]['tiny'] += 1
            elif ratio <= object_info[1]:
                classes_dict[classes[int(float(cls_id))]]['s'] += 1
            elif ratio <= object_info[2]:
    ...
    # 统计总和
    total_tiny = sum(v['tiny'] for v in classes_dict.values())
    ...
    # 创建表格
    table = PrettyTable()
    table.field_names = ["Category", "Tiny(≤0.03)", "Small (s)", "Medium (m)", "Large (l)", "Total (num)"]

    # 添加每一行
    for category, values in classes_dict.items():
        tiny, s, m, l, num = values['tiny'], values['s'], values['m'], values['l'], values['num']
        row = [
            category,
            f"{tiny} ({tiny / num:.1%})",
            f"{s} ({s / num:.1%})",
    ...
    row_total = [
        "All",
        f"{total_tiny} ({total_tiny / total_num:.1%})",
