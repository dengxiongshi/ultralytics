...
from ultralytics.utils import GitRepo, LOGGER, colorstr, YAML
...
def train(hyp, opt, device, callbacks):
    ...
    # Save run settings
    if not evolve:
        YAML.save(save_dir / "hyp.yaml", hyp)
        YAML.save(save_dir / "opt.yaml", vars(opt))
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    "git": {
                        ...
                        "commit": GIT.commit,
                        "origin": GIT.origin,
                    },  # {remote, branch, commit} if a git repo
