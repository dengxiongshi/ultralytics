...
from ultralytics.nn.modules import ShuffleNetV2, Conv_maxpool, GSConv, SimSPPF, A2C2f, MBConvBlock, stem, ADD, RIFusion
...
class VIAndIRDetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(2, 6, s, s))])  # forward
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        m_.i = i + 4 if is_backbone else i
        m_.f, m_.type = f, t  # attach index, 'from' index, type
        if verbose:
